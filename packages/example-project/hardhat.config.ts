...
import { configVariable, emptyTask, globalOption, task } from "hardhat/config"
	...
	tasks: [
		task("plugin-hello", "Prints a greeting from community-plugin")
			...
			.setAction(async ({ greeting }, _) => {
				...
				if (greeting === "") {
					throw new HardhatPluginError("community-plugin", "Greeting cannot be empty")
				}
			})
			.build(),
	],
...
const config: HardhatUserConfig = {
	...
	solidity: {
		...
		remappings: [
			...
			"forge-std/=npm/forge-std@1.9.7/src/",
		],
	},
	solidityTest: {
		testFail: true,
